...
def compute_csi_features(csi_string):
    try:
        ...
        return {'csi_mean': float(np.mean(amps)), 'csi_std': float(np.std(amps)), 
                'csi_max': float(np.max(amps)), 'csi_min': float(np.min(amps)), 
                'csi_range': float(np.max(amps) - np.min(amps)), 'csi_q25': float(np.percentile(amps, 25)), 
                'csi_q75': float(np.percentile(amps, 75)), 'csi_iqr': float(np.percentile(amps, 75) - np.percentile(amps, 25)), 
...
def process_data():
    # Scan for data in round folders
    ...
    # Computing CSI features BEFORE windowing
    ...
    for (block, label), group in full_df.groupby(['block_target', 'label']):
        ...
        for _, window in group.groupby(pd.Grouper(freq=f'{WINDOW_SIZE_SECONDS}s')):
            ...
            row = {
                ...
                'rssi_s1': s[0], 'rssi_s2': s[1], 'rssi_s3': s[2], 'rssi_s4': s[3], 
                'rel_s1': s[0]-rssi_mean, 'rel_s2': s[1]-rssi_mean, 'rel_s3': s[2]-rssi_mean, 'rel_s4': s[3]-rssi_mean, 
                'rssi_std': float(np.std(s)), 'rssi_range': max(s)-min(s), 
