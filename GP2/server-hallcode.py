...
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InR4Ym1kbmhveXhzY2tkbmJlZ2d3Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2MzQ3NDI2NywiZXhwIjoyMDc5MDUwMjY3fQ.OWt5frXgjD6ydcYmUYj8vKlTPmrT63LJhOU_GPNEayE"
...
try:
    ...
    print(f"  Supabase     : NOT CONNECTED ✗  ({e})")

# Session cache — avoids a Supabase query on every UDP packet
# { hall_code → (session_id_or_None, cache_timestamp) }
_session_cache: dict = {}
SESSION_CACHE_TTL = 30   # seconds before re-querying

def get_active_session_id(hall_code: str) -> str | None:
    """
    Return the UUID of the currently active session for hall_code.
    Result is cached for SESSION_CACHE_TTL seconds to avoid hammering Supabase.
    Returns None if no active session exists or Supabase is unavailable.
    """
    if supabase is None or not hall_code:
        return None
    now = time.time()
    if hall_code in _session_cache:
        sid, ts = _session_cache[hall_code]
    ...
    try:
        resp = (
            ...
            .select("id")
            .eq("status", "active")
            .eq("hall_code", hall_code)
            .order("actual_started_at", desc=True)
            .limit(1)
            .execute()
        )
        rows = resp.data or []
        sid  = rows[0]["id"] if rows else None
        _session_cache[hall_code] = (sid, now)
        if sid is None:
            print(f"  ⚠ No active session for hall_code='{hall_code}'")
        return sid
    except Exception as e:
...
def push_to_db(anchor_id: str, rssi: int, mac: str, current_zone: str, hall_code: str):
    """
    Insert one detection row into espData.
    block_number is parsed from current_zone (e.g. 'Block_3' → 3).
    session_id is resolved via get_active_session_id (cached).
    If session_id is None the DB trigger will attempt fallback assignment.
    """
    if supabase is None:
        return
    ...
    if current_zone and "Block" in current_zone:
        try:
            block_num = int("".join(filter(str.isdigit, current_zone)))
        except ValueError:
            block_num = 0

    session_id = get_active_session_id(hall_code)

    try:
        supabase.table("espData").insert({
            "anchor_id":    anchor_id,
            "ssid":         "RAQEEB_SYSTEM",
            "rssi":         rssi,
...
def compute_csi_features(csi_string):
    """
    Parse raw CSI I/Q pairs from ESP32 and compute 9 amplitude features.
    Input: space-separated integers e.g. "0 1 -3 2 5 -1 ..."
    Each pair (real, imaginary) → amplitude = sqrt(real² + imag²)
    """
    try:
...
# ─────────────────────────────────────────────────────────────────
# FALLBACK — threshold logic if models.pkl missing
# ─────────────────────────────────────────────────────────────────
...
active_devices   = {}
last_update_time = 0
_state_lock      = threading.Lock()   # guards active_devices across UDP + Flask threads
...
# ─────────────────────────────────────────────────────────────────
# UDP LISTENER
# Packet format from sniffer-hallcode.cpp:
#   DATA,{sniffer_id},{hall_code},{timestamp_us},{rssi},{csi I/Q pairs...}
# ─────────────────────────────────────────────────────────────────
def process_udp_packet(raw: str):
    ...
    try:
        ...
        # Split into at most 6 parts so the CSI field stays intact
        parts = raw.split(",", 5)
        if len(parts) < 5:
            return

        sniffer_id = int(parts[1])
        hall_code  = parts[2].strip()
        # parts[3] = timestamp_us — not needed for DB, ignored
        rssi_val   = int(parts[4])
        csi_string = parts[5] if len(parts) == 6 else None
        ...
        # Push to Supabase outside the lock (network I/O should not block the lock)
        push_to_db(anchor, rssi_val, mac, current_zone, hall_code)
...
# ─────────────────────────────────────────────────────────────────
# FLASK ENDPOINT — legacy HTTP path (hall_code in JSON body)
# ─────────────────────────────────────────────────────────────────
        ...
        if mac and mac.lower() == TARGET_MAC.lower():
            with _state_lock:
                ...
                if anchor and rssi:
                    ...
                    push_to_db(anchor, rssi_val, mac.lower(), current_zone, hall_code)
